...
void UpdateM(std::array< MultiFab, AMREX_SPACEDIM >& Mfield, std::array< MultiFab, AMREX_SPACEDIM >& LLG_RHS, std::array< MultiFab, AMREX_SPACEDIM >& Ms, Real dt)
{
        //for (MFIter mfi(*Mfield[0]); mfi.isValid(); ++mfi)
        for (MFIter mfi(Mfield[0]); mfi.isValid(); ++mfi)
        {
            ...
            amrex::ParallelFor( tbx, [=] AMREX_GPU_DEVICE (int i, int j, int k)
            {
                if (Ms_xface_arr(i,j,k) > 0._rt)
                { 
                   M_xface(i, j, k, 0) += dt * LLG_rhs_xface(i, j, k, 0); 
                   M_xface(i, j, k, 1) += dt * LLG_rhs_xface(i, j, k, 1); 
                   M_xface(i, j, k, 2) += dt * LLG_rhs_xface(i, j, k, 2); 
                }
            });     
            ...
            amrex::ParallelFor( tby, [=] AMREX_GPU_DEVICE (int i, int j, int k)
            {
                if (Ms_yface_arr(i,j,k) > 0._rt)
                {
                   M_yface(i, j, k, 0) += dt * LLG_rhs_yface(i, j, k, 0); 
                   M_yface(i, j, k, 1) += dt * LLG_rhs_yface(i, j, k, 1); 
                   M_yface(i, j, k, 2) += dt * LLG_rhs_yface(i, j, k, 2); 
                }
            });     
            ...
            amrex::ParallelFor( tbz, [=] AMREX_GPU_DEVICE (int i, int j, int k)
            {
                if (Ms_zface_arr(i,j,k) > 0._rt)
                {
                   M_zface(i, j, k, 0) += dt * LLG_rhs_zface(i, j, k, 0); 
                   M_zface(i, j, k, 1) += dt * LLG_rhs_zface(i, j, k, 1); 
                   M_zface(i, j, k, 2) += dt * LLG_rhs_zface(i, j, k, 2); 
                }
            });    
...
void UpdateM_2nd(std::array< MultiFab, AMREX_SPACEDIM >& Mfield, std::array< MultiFab, AMREX_SPACEDIM >& LLG_RHS, std::array< MultiFab, AMREX_SPACEDIM >& LLG_RHS_pre, std::array< MultiFab, AMREX_SPACEDIM >& Ms, Real dt)
{
        //for (MFIter mfi(*Mfield[0]); mfi.isValid(); ++mfi)
        for (MFIter mfi(Mfield[0]); mfi.isValid(); ++mfi)
        {
            ...
            const Array4<Real>& LLG_rhs_pre_xface = LLG_RHS_pre[0].array(mfi);
            const Array4<Real>& LLG_rhs_pre_yface = LLG_RHS_pre[1].array(mfi);
            const Array4<Real>& LLG_rhs_pre_zface = LLG_RHS_pre[2].array(mfi);
            ...
            amrex::ParallelFor( tbx, [=] AMREX_GPU_DEVICE (int i, int j, int k)
            {
                if (Ms_xface_arr(i,j,k) > 0._rt)
                { 
                   M_xface(i, j, k, 0) += 0.5 * dt * (LLG_rhs_xface(i, j, k, 0) + LLG_rhs_pre_xface(i, j, k, 0)); 
                   M_xface(i, j, k, 1) += 0.5 * dt * (LLG_rhs_xface(i, j, k, 1) + LLG_rhs_pre_xface(i, j, k, 1)); 
                   M_xface(i, j, k, 2) += 0.5 * dt * (LLG_rhs_xface(i, j, k, 2) + LLG_rhs_pre_xface(i, j, k, 2)); 
                }
            });     
            ...
            amrex::ParallelFor( tby, [=] AMREX_GPU_DEVICE (int i, int j, int k)
            {
                if (Ms_yface_arr(i,j,k) > 0._rt)
                {
                   M_yface(i, j, k, 0) += 0.5 * dt * (LLG_rhs_yface(i, j, k, 0) + LLG_rhs_pre_yface(i, j, k, 0)); 
                   M_yface(i, j, k, 1) += 0.5 * dt * (LLG_rhs_yface(i, j, k, 1) + LLG_rhs_pre_yface(i, j, k, 1)); 
                   M_yface(i, j, k, 2) += 0.5 * dt * (LLG_rhs_yface(i, j, k, 2) + LLG_rhs_pre_yface(i, j, k, 2)); 
                }
            });     
            ...
            amrex::ParallelFor( tbz, [=] AMREX_GPU_DEVICE (int i, int j, int k)
            {
                if (Ms_zface_arr(i,j,k) > 0._rt)
                {
                   M_zface(i, j, k, 0) += 0.5 * dt * (LLG_rhs_zface(i, j, k, 0) + LLG_rhs_pre_zface(i, j, k, 0)); 
                   M_zface(i, j, k, 1) += 0.5 * dt * (LLG_rhs_zface(i, j, k, 1) + LLG_rhs_pre_zface(i, j, k, 1)); 
                   M_zface(i, j, k, 2) += 0.5 * dt * (LLG_rhs_zface(i, j, k, 2) + LLG_rhs_pre_zface(i, j, k, 2)); 
                }
            });    
